...
@property (nonatomic, retain, readwrite) NSString *firstAddress;
@property (nonatomic, retain, readwrite) NSString *fullAddress;
...
@synthesize name, reference, identifier, type,firstAddress,fullAddress;
...
+ (SPGooglePlacesAutocompletePlace *)placeFromDictionary:(NSDictionary *)placeDictionary {
    ...
    NSArray *arrTemp = [placeDictionary objectForKey:@"terms"];
    
    if([arrTemp count]>0)
    {
        place.name = [[arrTemp objectAtIndex:0] objectForKey:@"value"];
    }
    else
    {
        place.name = @"";
    ...
    if([arrTemp count]>1)
    {
        place.firstAddress =[[arrTemp objectAtIndex:1] objectForKey:@"value"];
    }
    else
    {
        place.firstAddress = @"";
    }
    place.fullAddress = [placeDictionary objectForKey:@"description"];
